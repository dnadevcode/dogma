function [barcodeIslands,barcodeIslandsData,badData,badDataInfo,barIslands] = create_barset(pscores,NN,barcodeGen,overlapStruct,synthStr)
...
%     NN=13
    for idxPair = 1:NN
        ...
        barIslands{idxPair}.curPair =  [curSink curSource];
%         
...
%         fig1 = plot_best_pos([], synthStr([curSink curSource]), [], [], [],10000,1);%1/bpPx*10^6
        ...
        cS = overlapStruct(curSink,curSource);
        
%         -cS.pA+1+cS.lenA-1
%         -cS.pB+1+cS.lenB-1

        % consistency checks: 1) orientation, 2) position, 3) resize factor 4)
        % local score
...
%                 -cS.pA+1 -cS.pA+1+cS.lenA-1 cS.or cS.bestBarStretch];
                ...
                [barcodeIslandsData, barcodeIslands,badData,badDataInfo{idxPair}] =  update_barcode_members_info_source_sink(cS,badData,barcodeIslands,barcodeIslandsData,sinkGroup,sourceGroup,pos1,pos2,curSource,curSink);
...
function [barcodeIslandsData, barcodeIslands, badData, badDataInfo ] =  update_barcode_members_info_source_sink(cS,badData,barcodeIslands,barcodeIslandsData,sinkGroup,sourceGroup, pos1,pos2, source, sink)
...
%      oSpair = test_plot_new_edge(barStruct, tempPosData,source,sink);
    ...
    if sinkGroup==sourceGroup % if same group, we already aligned 
        ...
        if abs(positionalDifference) > 50 % potentially also remove these elements from bargrouping islands altogether since they are maching ambiguosly
            badData = [badData; source sink];
