function [maxPCC,totLen,barcodes2] = gen_scores_chrom(chrom,...
    NUM_RAND_FRAGMENTS,RAND_LENGTH_MIN,RAND_LENGTH_2,MIN_OVERLAP_PIXELS,...
    NN,sF,out,SCAMP_LINE)

% gen scores for chrom
% NN=100;
mkdir(out);
% parpool('local',28)
mp1 = cell(1,NN);
...
allBars = 1:length(chrom)-2000;
allBars(1:2000) = nan;
for i=2000:length(chrom)-4000
    if isnan(chrom(i))
        allBars(i-2000+1:i+2000-1) = nan;
    end
end
usableIdx = find(allBars);
...
%     tic
    ...
    barcodes = arrayfun(@(x) chrom(usableIdx(x):usableIdx(x)+RAND_LENGTH_MIN+(k-1)*10-1), randi(max(usableIdx),NUM_RAND_FRAGMENTS,1),'un',false)';
%     [barcodes] = gen_random(NUM_RAND_FRAGMENTS,PSF_WIDTH_PIXELS,RAND_LENGTH_MIN+(k-1)*10,0);
    ...
    barcodes2 = arrayfun(@(x) chrom(usableIdx(x):usableIdx(x)+RAND_LENGTH_2-1), randi(max(usableIdx),NUM_RAND_FRAGMENTS,1),'un',false)';
...
%     tic
    command = strcat([SCAMP_LINE ' --window=' num2str(MIN_OVERLAP_PIXELS) ' --input_a_file_name='...
       names2{1} ' --input_b_file_name=' namesBar{1} ' --num_cpu_workers=4 --no_gpu --output_pearson --print_debug_info'...
